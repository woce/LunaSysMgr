...
void RemoteWindowDataSoftware::prepareSceneTransition(CardWebApp* cardWebApp)
{
	...
	PGSurface* surf = NativeGraphicsSurface::wrap(m_width,
												  m_height,
												  (unsigned char*) m_transitionIpcBuffer->data(),
												  false);
	...
	gc->setSurface(surf);
	cardWebApp->paint(gc, surf, tx, ty, tw, th, tx, ty, tw, th);
...
void RemoteWindowDataSoftware::prepareCrossAppSceneTransition()
{
	m_channel->sendAsyncMessage(new ViewHost_SceneTransitionPrepareCrossApp(key()));
}

void RemoteWindowDataSoftware::cleanupSceneTransition()
{
	WindowMetaData* metaData = (WindowMetaData*) m_metaDataBuffer->data();
	m_metaDataBuffer->lock();
	metaData->transitionBufferKey = 0;
	m_metaDataBuffer->unlock();

	if (m_transitionIpcBuffer) {
		delete m_transitionIpcBuffer;
		m_transitionIpcBuffer = 0;
	}
}
