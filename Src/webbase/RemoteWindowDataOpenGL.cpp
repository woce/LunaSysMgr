...
void RemoteWindowDataOpenGL::prepareSceneTransition(CardWebApp* cardWebApp)
{
	if (m_transitionIpcBuffer) {
		// in case prepare gets called twice in a row
		delete m_transitionIpcBuffer;
	}

	m_transitionIpcBuffer = PIpcBuffer::create(m_width * m_height * sizeof(uint32_t));
	memset((uint32_t*) m_transitionIpcBuffer->data(), 0xFF, m_width * m_height * sizeof(uint32_t));

	PGContext* gc = renderingContext();
	int tx = 0;
	int ty = 0;
	int tw = m_width;
	int th = m_height;

	// Make sure to update the window surface
	cardWebApp->paint(gc, m_surface, tx, ty, tw, th, tx, ty, tw, th);

	PGSurface* tmpSurf = gc->createSurface(m_width, m_height, false);
	gc->setSurface(tmpSurf);
	cardWebApp->paint(gc, tmpSurf, tx, ty, tw, th, tx, ty, tw, th);

	uint32_t* imageBuffer = PrvGetImageBuffer();
	m_context->pgContext()->GetPixels(PRect(0, 0, m_width, m_height), imageBuffer);
	memcpy(m_transitionIpcBuffer->data(), imageBuffer, m_width * m_height * 4);	

	tmpSurf->releaseRef();

	gc->setSurface(m_surface);
	
	WindowMetaData* metaData = (WindowMetaData*) m_metaDataBuffer->data();
	m_metaDataBuffer->lock();
	metaData->transitionBufferKey = m_transitionIpcBuffer->key();
	m_metaDataBuffer->unlock();

	m_channel->sendAsyncMessage(new ViewHost_SceneTransitionPrepare(key(), m_width, m_height));
}

void RemoteWindowDataOpenGL::prepareCrossAppSceneTransition()
{
    
}

void RemoteWindowDataOpenGL::runSceneTransition(CardWebApp* cardWebApp, const char* transitionType,
...
void RemoteWindowDataOpenGL::crossAppSceneActive(CardWebApp* cardWebApp, int appBufWidth, int appBufHeight)
{
    
}

void RemoteWindowDataOpenGL::cleanupSceneTransition()
{
