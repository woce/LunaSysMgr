...
public:
	...
	virtual void endPaint(bool preserveOnFlip, const PRect& rect, bool flipBuffers = true) = 0;
	...
	virtual void prepareSceneTransition(CardWebApp* cardWebApp) = 0;
	virtual void prepareCrossAppSceneTransition() = 0;
	virtual void runSceneTransition(CardWebApp* cardWebApp, const char* transitionType,
			bool isPop, int appBufWidth, int appBufHeight) = 0;
	virtual void crossAppSceneActive(CardWebApp* cardWebApp, int appBufWidth, int appBufHeight) = 0;

	virtual void cleanupSceneTransition() = 0;

	void setChannel(PIpcChannel* channel) { m_channel = channel; }

	void setTransitionIpcBuffer(PIpcBuffer* transitionIpcBuffer) { m_transitionIpcBuffer = transitionIpcBuffer; }
	PIpcBuffer* getTransitionIpcBuffer() { return m_transitionIpcBuffer; }
...
protected:
	...
	PIpcBuffer* m_transitionIpcBuffer;
