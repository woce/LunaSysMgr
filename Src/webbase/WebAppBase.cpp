...
#if defined (TARGET_DEVICE)
    HALConnectorBase::Sensor halSensorType = mapSensorType(aSensorType);

    if (aNeedEvents)
    {
        isSuccess = enableSensor(halSensorType);
    }
    else
    {
        isSuccess = disableSensor(halSensorType);
    }
#endif
    return isSuccess;
}
...
{
#if defined (TARGET_DEVICE)
    if (enable && enableSensor(HALConnectorBase::SensorAcceleration))
    {
        if (m_SensorList.contains(HALConnectorBase::SensorAcceleration))
        {
            HALConnectorBase *pSensor = m_SensorList.value(HALConnectorBase::SensorAcceleration);
            pSensor->setRate(HALConnectorBase::SensorReportRateHighest);
        }
    }
#endif
}
...
{
    QMap<HALConnectorBase::Sensor, QPointer<HALConnectorBase> >::iterator itr = m_SensorList.begin();
    for (; itr != m_SensorList.end(); ++itr)
    {
        HALConnectorBase *pSensor = itr.value();
        delete pSensor;
        pSensor = 0;
    }
...
bool WebAppBase::enableSensor(HALConnectorBase::Sensor aSensorType)
{
    bool isSuccess = false;
    ...
    {
        // Create the sensor
        pSensor = HALConnectorBase::getSensor(aSensorType, this);
        if (pSensor)
        {
...
bool WebAppBase::disableSensor(HALConnectorBase::Sensor aSensorType)
{
    bool isSuccess = false;

    if (m_SensorList.contains(aSensorType))
    {
        HALConnectorBase *pSensor = m_SensorList.value(aSensorType);
...
void WebAppBase::HALDataAvailable (HALConnectorBase::Sensor aSensorType)
{
    bool eventHandled = false;

    switch (aSensorType)
    {
        case HALConnectorBase::SensorAcceleration:
        {
            eventHandled = sendAccelerationEvent();
            break;
        }

        case HALConnectorBase::SensorShake:
        {
            eventHandled = sendShakeEvent();
            break;
        }

        case HALConnectorBase::SensorLogicalOrientation:
        {
            eventHandled = sendLogicalOrientationEvent();
            break;
        }

        case HALConnectorBase::SensorBearing:
        {
...
bool WebAppBase::sendCompassEvent()
{
    ...
    if (m_SensorList.contains(HALConnectorBase::SensorBearing))
    {
        HALConnectorBase *pSensor = m_SensorList.value(HALConnectorBase::SensorBearing);
        if (pSensor)
        {
            HALBearingSensorConnector *pBearingSensor = static_cast<HALBearingSensorConnector *>(pSensor);
...
bool WebAppBase::sendAccelerationEvent()
{
    ...
    if (m_SensorList.contains(HALConnectorBase::SensorAcceleration))
    {
        ...
        if (pSensor)
        {
            HALAccelerationSensorConnector *pAccelerationSensor = static_cast<HALAccelerationSensorConnector *>(pSensor);
...
bool WebAppBase::sendShakeEvent()
{
    ...
    if (m_SensorList.contains(HALConnectorBase::SensorShake))
    {
        HALConnectorBase *pSensor = m_SensorList.value(HALConnectorBase::SensorShake);
        if (pSensor)
        {
...
bool WebAppBase::sendLogicalOrientationEvent()
{
    ...
    if (m_SensorList.contains(HALConnectorBase::SensorLogicalOrientation))
    {
        HALConnectorBase *pSensor = m_SensorList.value(HALConnectorBase::SensorLogicalOrientation);
        if (pSensor)
        {
            HALLogicalOrientationSensorConnector *pLogicalOrientationSensor = static_cast<HALLogicalOrientationSensorConnector *>(pSensor);
...
HALConnectorBase::Sensor WebAppBase::mapSensorType(Palm::SensorType aSensorType)
{
    ...
    switch(aSensorType)
    {
        ...
        case Palm::SensorOrientation:
        {
            ...
            mappedSensor = HALConnectorBase::SensorLogicalOrientation;
            break;
        }
