...
WebAppBase* WebAppFactoryLuna::createWebApp(Window::Type winType, PIpcChannel* channel, ApplicationDescription* desc)
{
	...
	case (Window::Type_PopupAlert):
		app = new AlertWebApp(std::string(), -1, -1, winType, channel);
		break;
	default:
...
WebAppBase* WebAppFactoryLuna::createWebApp(Window::Type winType, WebPage* page, PIpcChannel* channel, ApplicationDescription* desc)
{
	WebAppBase* app = 0;
	...
	case (Window::Type_BannerAlert):
		if(true == m_dashboardOwnsNegativeSpace)
			app = new AlertWebApp(page->appId(), page->requestedWidth(), page->requestedHeight(), winType, channel);
		else
			app = new AlertWebApp(page->appId(), kTableUiAlertWidth, page->requestedHeight(), winType, channel);
		break;
	case (Window::Type_Launcher):
	case (Window::Type_Menu): {
		app = new WindowedWebApp(WebAppManager::instance()->currentUiWidth(),
...
WebAppBase* WebAppFactoryLuna::createWebApp(Window::Type winType, int width, int height, PIpcChannel* channel, ApplicationDescription* desc)
{
	...
	case (Window::Type_PopupAlert):
		if(true == m_dashboardOwnsNegativeSpace)
			app = new AlertWebApp(std::string(), WebAppManager::instance()->currentUiWidth(), height, winType, channel);
		else
			app = new AlertWebApp(std::string(), kTableUiAlertWidth, height, winType, channel);
		break;
	default:
