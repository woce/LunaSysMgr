...
CardWindow::CardWindow(Window::Type type, HostWindowData* data, IpcClientHost* clientHost)
	...
	, m_loadingTimerId(0)
	, m_transition(0)
	, m_transitionTimer(HostBase::instance()->masterTimer(), this, &CardWindow::transitionTimerTicked)
...
CardWindow::~CardWindow()
{
	...
	if (m_data) {
		// Dock window can be created without m_data
...
void CardWindow::queueUpWindowForFlip(QRect windowScreenBounds, bool sync)
{
	...
	if(pix) {
		...
		if ((m_adjustmentAngle != 90 && m_adjustmentAngle != -90) || this->type() == Window::Type_Emulated_Card){
...
void CardWindow::sceneTransitionPrepare()
{
	// the webkit side turns direct rendering off before it calls prepareSceneTransition on the sysmgr side
	// we unset this flag here to match the state of the webkit side and save an IPC back-and-forth
	m_fullScreenEnabled = false;
	m_fullScreenModeSuppressed = true;
...
void CardWindow::sceneTransitionRun(const std::string& transitionName)
{
	if (!m_data->acquireTransitionPixmap()) {
		// If CardWebApp sent a run-transition call and we don't have
		// a current transition running, then we need to make sure to
		// send a scene transition finished call otherwise the webapp
		// will be stuck in StateRunTransition State
		m_fullScreenModeSuppressed = false;
		if (!m_transition) {
			if (m_channel) {
				m_channel->sendAsyncMessage(new View_SceneTransitionFinished(routingId()));
...
void CardWindow::sceneTransitionCancel()
{
	...
	// Need to trigger update here since the transition was canceled
	update();
}

void CardWindow::sceneTransitionPrepareCrossApp()
{
	// the webkit side turns direct rendering off before it calls sceneTransitionPrepareCrossApp on the sysmgr side
	// we unset this flag here to match the state of the webkit side
	m_fullScreenEnabled = false;
	m_fullScreenModeSuppressed = true;
}

bool CardWindow::transitionTimerTicked()
{
	bool ret = true;

	if (G_LIKELY(m_transition)) {

		if (m_transition->finished()) {
			m_transition->markCompleted(true);
			m_transitionTimer.stop();
			m_fullScreenModeSuppressed = false;
			if (m_channel) {
				m_channel->sendAsyncMessage(new View_SceneTransitionFinished(routingId()));
...
void CardWindow::setVisibleDimensions(int width, int height)
{
	if(!m_isResizing && m_flipsQueuedUp) {
		if (m_data && (type() != Window::Type_Emulated_Card) && !isHost() &&
	...
	if ((m_adjustmentAngle != 90 && m_adjustmentAngle != -90) || (this->type() == Window::Type_Emulated_Card || this->type() == Window::Type_ModalChildWindowCard)) {
...
void CardWindow::paintBase(QPainter* painter, bool maximized)
{
	if (G_UNLIKELY(m_transition)) {
		...
		if (G_LIKELY(maximized)) {
			painter->setRenderHint(QPainter::SmoothPixmapTransform, false);
			painter->fillRect(boundingRect(), *m_data->acquireTransitionPixmap());
		} else {
			painter->fillPath(m_paintPath, *m_data->acquireTransitionPixmap());
		}
...
void CardWindow::initializeRoundedCornerStage()
{
	...
	if (!m_isResizing && !m_flipsQueuedUp && m_data && (type() != Window::Type_Emulated_Card) && !isHost() &&
		(type() != Window::Type_ModalChildWindowCard) && (m_appFixedOrientation == Event::Orientation_Invalid)) {
		// safeguard code in case the data buffer and the bounding rect dimensions get out of sync
		bool isDataLandscape = m_data->width() >= m_data->height();
		bool isBoundingRectLandscape = m_boundingRect.width() >= m_boundingRect.height();
...
void CardWindow::onSetAppFixedOrientation(int orientation, bool isPortrait) {
	...
	if(orientation != Event::Orientation_Invalid) {
		requiresBufferResize = (isPortrait != currBuffPortrait) && (m_type != Window::Type_Emulated_Card) && (!isHost());
	} else {
		if(m_appFixedOrientation!= Event::Orientation_Invalid) {
			requiresBufferResize = uiIsInPortraitMode != currBuffPortrait && (m_type != Window::Type_Emulated_Card) && (!isHost());
		}
	}
