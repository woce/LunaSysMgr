...
void CardHostWindow::drawInRotateAnim(PGContext* gc, bool maximized)
{
#ifdef FIX_FOR_QT
#else
	int sx = screenX();
	int sy = screenY();
	int sw = Region.Length.X;
	int sh = Region.Length.Y;

	if (G_UNLIKELY(m_rotateTimerStart == 0)) {
		m_rotateTimerStart = Time::curTimeMs();
	}

	PValue dt = (int) (Time::curTimeMs() - m_rotateTimerStart);
	int durationMs = AS(cardTransitionDuration);
	if (durationMs < (int) dt)
		dt = durationMs;

	gc->push();
	gc->translate(sx, sy);
	gc->setStrokeColor(PColor32(0, 0, 0, 0));
	gc->setFillColor(PColor32(0, 0, 0, 0xFF));
	gc->drawRect(0, 0, sw, sh);
	gc->pop();

	int angle = m_rotateEquation(dt, m_rotateAngleStart, m_rotateAngleTarget,
								 durationMs, AS(cardTransitionCurve));


	PValue scale = m_rotateEquation(dt, m_rotateScaleStart, m_rotateScaleTarget,
									durationMs, AS(cardTransitionCurve));

	gc->push();

	gc->addClipRect(sx, sy, sx + sw, sy + sh);

	gc->translate(sx, sy);

	gc->translate(sw/2, sh/2);
	gc->rotate(angle);
	gc->translate(-(scale * sw/2), -(scale * sh/2));
	gc->scale(scale, scale);

	gc->bitblt(m_surf, 0, 0, m_visibleWidth, m_visibleHeight,
			   0, 0, sw, sh);
