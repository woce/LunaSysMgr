...
ApplicationDescription* ApplicationDescription::fromFile(const std::string& filePath, const std::string& folderPath)
{
	...
	// UI Revision 1: legacy phone UI support
	// UI Revision 2: Topaz tablets and forward
	label = json_object_object_get(root, "uiRevision");
	if( label && !is_error(label) )
	{
		appDesc->m_uiRevision = json_object_get_int(label);
		if (appDesc->m_uiRevision < 1) {
			appDesc->m_uiRevision = 1;
		} else if (appDesc->m_uiRevision > 2) {
			appDesc->m_uiRevision = 2;
		}
	}
...
ApplicationDescription* ApplicationDescription::fromJsonString(const char* jsonStr)
{
	...
	success &= extractFromJson(root, "uiRevision", temp);
	appDesc->m_uiRevision = temp;
	if (appDesc->m_uiRevision < 1) {
		appDesc->m_uiRevision = 1;
	} else if (appDesc->m_uiRevision > 2) {
		appDesc->m_uiRevision = 2;
...
json_object* ApplicationDescription::toJSON() const
{
	...
	json_object_object_add(json, (char*) "uiRevision",json_object_new_int(this->m_uiRevision));
...
bool ApplicationDescription::strictCompare(const ApplicationDescription& cmp) const {
	...
	if (m_appSize != cmp.m_appSize)
		return false;
	if (m_uiRevision != cmp.m_uiRevision)
		return false;
...
int ApplicationDescription::update(const ApplicationDescription& appDesc) 
{
	...
	m_uiRevision = appDesc.m_uiRevision;
...
void ApplicationDescription::getAppDescriptionString(std::string &descString) const
{
	...
	json_object_object_add(json, (char*) "uiRevision", json_object_new_int(m_uiRevision));
