...
#include "Logging.h"
#include "Settings.h"
#include "pagesaver.h" // for launcher3 saving
...
bool BackupManager::preBackupCallback( LSHandle* lshandle, LSMessage *message, void *user_data)
{
	...
	if (pThis->m_doBackupCookies) {
	    bool succeeded = Palm::WebGlobal::startDatabaseDump (Palm::k_PhonyCookieUrl, "cookies", strCookieTempFile, NULL);
...
bool BackupManager::postRestoreCallback( LSHandle* lshandle, LSMessage *message, void *user_data)
{
	...
	for (index = 0; index < fileArrayLength; ++index) {
		...
		bool succeeded = Palm::WebGlobal::startDatabaseRestore(Palm::k_PhonyCookieUrl, "cookies", path, NULL);
		...
		bool succeeded = Palm::WebGlobal::startDatabaseRestore(strBrowserDbUrl, "browser_data", path, NULL);
		if (!succeeded) {
