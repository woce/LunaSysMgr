...
HapticsControllerCastle::HapticsControllerCastle()
{
    ...
    error = hal_device_open(HAL_DEVICE_HAPTICS, "Main", &d);
...
{
    hal_error_t error = HAL_ERROR_SUCCESS;
    haptics_configuration_t configuration;

	int index = 0;
	int id;
	if(strcmp(name,"ringtone") == 0) {
		configuration.type = HAPTICS_EFFECT_RINGTONE;
	} else if(strcmp(name,"alert") == 0) { //long
		configuration.type = HAPTICS_EFFECT_ALERT;
	} else if(strcmp(name, "notification") == 0) { //short
        configuration.type = HAPTICS_EFFECT_NOTIFICATION;
	} else if(strcmp(name, "tapdown") == 0) {
        configuration.type = HAPTICS_EFFECT_TAPDOWN;
	} else if(strcmp(name, "tapup") == 0) {
        configuration.type = HAPTICS_EFFECT_TAPUP;
	} else {
		return -1;
	}

    error = hal_haptics_vibrate(d, configuration);

    if (error != HAL_ERROR_SUCCESS)
        g_critical("Failed on hal_haptics_vibrate!");
    else
    {
        error = hal_haptics_get_effect_id(d, &id);
        if (error != HAL_ERROR_SUCCESS)
...
int HapticsControllerCastle::vibrate(int period,int duration)
{
    ...
    hal_error_t error = HAL_ERROR_SUCCESS;
    haptics_configuration_t configuration;
    configuration.type = HAPTICS_EFFECT_UNDEFINED;
...
void HapticsControllerCastle::cancelAll() 
{
    ...
    hal_error_t error = HAL_ERROR_SUCCESS;
    error = hal_haptics_cancel_all(d);

    if (error != HAL_ERROR_SUCCESS)
        g_critical("Failed on hal_haptics_cancel_all!");
}
