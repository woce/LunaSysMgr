...
std::string DeviceInfo::jsonString(Window::Type appType) {
	if (appType == Window::Type_Emulated_Card) {
		//return a special JSON string if the app is using emulation mode
		return m_emuJsonString;
	} else {
		return jsonString();
	}
}
...
void DeviceInfo::gatherInfo()
{
	...
	m_emuScreenWidth = settings->emulatedCardWidth;
	m_emuScreenHeight = settings->emulatedCardHeight;
	...
	m_emuMaximumCardWidth = m_emuScreenWidth;
	m_emuMaximumCardHeight = m_emuScreenHeight - settings->positiveSpaceTopPadding;

	m_emuMinimumCardWidth = m_emuScreenWidth;
	m_emuMinimumCardHeight = m_emuScreenHeight - settings->positiveSpaceTopPadding -
						  maxNegativeSpaceHeight;
	...
	m_emuTouchableRows = (m_emuScreenHeight
					   - settings->positiveSpaceTopPadding
					   - settings->positiveSpaceBottomPadding) / kTouchableHeight;
...
*/
	...
	// Compose emu-mode json string from the parameters  -------------------------------

	json_object* emuJson = json_object_new_object();

	json_object_object_add(emuJson, (char*) "modelName", json_object_new_string(m_modelName.c_str()));
	json_object_object_add(emuJson, (char*) "modelNameAscii", json_object_new_string(m_modelNameAscii.c_str()));
	json_object_object_add(emuJson, (char*) "platformVersion", json_object_new_string(m_platformVersion.c_str()));
	json_object_object_add(emuJson, (char*) "platformVersionMajor", json_object_new_int (m_platformVersionMajor));
	json_object_object_add(emuJson, (char*) "platformVersionMinor", json_object_new_int (m_platformVersionMinor));
	json_object_object_add(emuJson, (char*) "platformVersionDot", json_object_new_int (m_platformVersionDot));
	json_object_object_add(emuJson, (char*) "carrierName", json_object_new_string(m_carrierName.c_str()));
	json_object_object_add(emuJson, (char*) "serialNumber", json_object_new_string(m_serialNumber.c_str()));

	json_object_object_add(emuJson, (char*) "screenWidth", json_object_new_int(m_emuScreenWidth));
	json_object_object_add(emuJson, (char*) "screenHeight", json_object_new_int(m_emuScreenHeight));

	json_object_object_add(emuJson, (char*) "minimumCardWidth", json_object_new_int(m_emuMinimumCardWidth));
	json_object_object_add(emuJson, (char*) "minimumCardHeight", json_object_new_int(m_emuMinimumCardHeight));
	json_object_object_add(emuJson, (char*) "maximumCardWidth", json_object_new_int(m_emuMaximumCardWidth));
	json_object_object_add(emuJson, (char*) "maximumCardHeight", json_object_new_int(m_emuMaximumCardHeight));

	json_object_object_add(emuJson, (char*) "touchableRows", json_object_new_int(m_emuTouchableRows));

	json_object_object_add(emuJson, (char*) "keyboardAvailable", json_object_new_boolean(m_keyboardAvailable));
	json_object_object_add(emuJson, (char*) "keyboardSlider", json_object_new_boolean(m_keyboardSlider));
	json_object_object_add(emuJson, (char*) "keyboardType", json_object_new_string(m_keyboardType.c_str()));

	json_object_object_add(emuJson, (char*) "wifiAvailable", json_object_new_boolean(m_wifiAvailable));
	json_object_object_add(emuJson, (char*) "bluetoothAvailable", json_object_new_boolean(m_bluetoothAvailable));

	json_object_object_add(emuJson, (char*) "carrierAvailable", json_object_new_boolean(carrierAvailable()));

	json_object_object_add(emuJson, (char*) "coreNaviButton", json_object_new_boolean(m_coreNaviButton));

	json_object_object_add(emuJson, (char*) "swappableBattery", json_object_new_boolean(m_swappableBattery));
	json_object_object_add(emuJson, (char*) "dockModeEnabled", json_object_new_boolean(true));

	m_emuJsonString = json_object_to_json_string(emuJson);

	json_object_put(emuJson);
