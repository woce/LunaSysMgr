...
#define URI_POWER_BACKLIGHT_SET "palm://com.palm.power/backlight/set"
#define JSON_BACKLIGHT_DISPLAY  "{\"display\":{\"brightness\":%i}}"
#define JSON_BACKLIGHT_KEYPAD   "{\"keypad\":{\"brightness\":%i}}"
#define JSON_BACKLIGHT_ALL      "{\"display\":{\"brightness\":%i,\"als\":%s},\"keypad\":{\"brightness\":%i}}"
...
bool DisplayManager::activity()
{
    ...
    g_debug ("%s: now=%i last=%i diff=%i", __FUNCTION__, now, m_lastEvent, now - m_lastEvent);

    if (now <  m_activityTimeout + m_lastEvent)
    {
        g_debug ("%s: restart the timer in %i", __FUNCTION__, m_activityTimeout + m_lastEvent - now);
...
void DisplayManager::backlightOn (int displayBrightness, int keyBrightness, bool als)
{
    ...
    gchar *brightness = g_strdup_printf (JSON_BACKLIGHT_ALL, displayBrightness, als ? "true" : "false", keyBrightness);
    g_message ("%s: calling %s with %s", __PRETTY_FUNCTION__, URI_POWER_BACKLIGHT_SET, brightness);

    if (!m_touchpanelIsOn)
        result = LSCallOneReply (m_service, URI_POWER_BACKLIGHT_SET, brightness, &DisplayManager::backlightOnCallback, this, NULL, &lserror);
    else
        result = LSCall (m_service, URI_POWER_BACKLIGHT_SET, brightness, NULL, this, NULL, &lserror);
...
bool DisplayManager::backlightOnCallback(LSHandle *sh, LSMessage *message, void *ctx)
{
...
void DisplayManager::setTouchpanelMode (bool active)
{
    ...
    {
	    if (active) {
		    g_message ("%s: Setting hal touchpanel mode to %d", __PRETTY_FUNCTION__, HAL_TOUCHPANEL_MODE_VIRTUAL_KEYBOARD);
		    err = hal_touchpanel_set_mode (ic->getHandle(), HAL_TOUCHPANEL_MODE_VIRTUAL_KEYBOARD);
	    }
	    else  {
		    g_message ("%s: Setting hal touchpanel mode to %d", __PRETTY_FUNCTION__, HAL_TOUCHPANEL_MODE_DEFAULT);
		    err = hal_touchpanel_set_mode (ic->getHandle(), HAL_TOUCHPANEL_MODE_DEFAULT);
	    }
       if (err) {
...
void DisplayManager::backlightOff()
{
    ...
    gchar *brightness = g_strdup_printf (JSON_BACKLIGHT_ALL, -1, "false", 0);
    g_message ("%s: calling %s with %s", __PRETTY_FUNCTION__, URI_POWER_BACKLIGHT_SET, brightness);
    result = LSCallOneReply (m_service, URI_POWER_BACKLIGHT_SET, brightness, &DisplayManager::backlightOffCallback, this, NULL, &lserror);
    if (!result)
    {
        g_message ("%s: failed at %s with message %s", __FUNCTION__, lserror.func, lserror.message);
        LSErrorFree(&lserror);
...
bool DisplayManager::backlightOffCallback (LSHandle *sh, LSMessage *message, void *ctx)
{
    EMPTY_SCHEMA_RETURN(sh, message);
...
bool DisplayManager::updateCompassBearingInfo(LSHandle* sh, LSMessage* message, void* ctx)
{
	...
	// Ensure that we have valid data - then call HAL and update them with the new values
	if(currentLatitude != latitude || currentLongitude != longitude)
    {
        result = DisplayManager::instance()->updateHalWithLocation(latitude, longitude);
        // If we updated HAL correctly, update our state as well
        if(result)
...
bool DisplayManager::updateHalWithLocation(double latitude, double longitude)
{
	bool result = false;

#if defined (TARGET_DEVICE)

    HALBearingSensorConnector *pBearingSensor = static_cast<HALBearingSensorConnector *> (HALConnectorBase::getSensor(HALConnectorBase::SensorBearing));
    if (pBearingSensor)
    {
...
bool DisplayManager::handleEvent(QEvent *event)
{
    ...
    if (mouseEvent && mouseEvent->type() == QEvent::MouseButtonRelease && mouseEvent->canceled()) {
	    g_debug ("Allowing canceled event at %d, %d through", mouseEvent->x(), mouseEvent->y());
		return false;
    }
    if (event->type() == QEvent::TouchEnd)
    {
    	g_debug ("Allowing touch-end event through");
    	return false;
    }
	...
	if (keyEvent && Qt::Key_Optical == keyEvent->key()) {
		if (keyEvent->type() == QEvent::KeyRelease) {
			g_debug("%s: optical up", __FUNCTION__);
...
void DisplayManager::changeDisplayState (DisplayState newState, DisplayState oldState, DisplayEvent displayEvent, sptr<Event> event)
{
    ...
    switch (newState) {
        ...
        case DisplayStateOffSuspended:
            ...
            break;
        default:
            g_warning ("unhandled state %d", newState);
            break;
    }
}
...
void DisplayManager::emitDisplayStateChange (int displaySignal)
{
	...
	case DISPLAY_SIGNAL_OFF_ON_CALL:
	    ...
	    break;
	default:
	    g_warning ("invalid display signal %d", displaySignal);
	    break;
    }
}
	...
	m_lockState = lockState;
            ...
            case DisplayLockDockMode:
		{
			...
			Q_EMIT signalLockStateChange (DISPLAY_DOCK_SCREEN, displayEvent);
        		...
        		break;
            default:
            	g_warning ("Unknown lock state %d", lockState);
                break;
        }
    }
}
